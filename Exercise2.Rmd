...
Save one file for hazard modeling, and one for ca(t) modeling, and save a nice life table.
...
setup_lifetable <- function(ptb){
  ptb %>% mutate(event = str_c("event", event)) %>%
          ...
          mutate(hazard = num((event1 / risk_set),digits=4)) %>% # calculate hazard estimate
          mutate(se_haz = num(sqrt((hazard * (1 - hazard)) / risk_set),digits=5)) %>% # se hazard
          group_by(condition) %>%
          mutate(survival = num(cumprod(1-hazard),digits=4), # calculate survival estimate
                 term     = num(cumsum(hazard / (risk_set * (1 - hazard))),digits=5),
                 se_surv  = num( (survival * sqrt(term)) , digits=5) ) %>% # Greenwood's (1926) approximation
          ungroup() 
}
```

```{r funcconditionalaccuracy}
...
```{r maketable}
ft <- flextable::flextable(data_final[[7]][[1]] %>% filter(condition==1) 
          %>% select(period,risk_set,hazard,se_haz,survival,se_surv,ca, se_ca))

# TODO: add row for time zero

# save in folder tables
save_as_image(ft, "tables/lifetable.png")
```

Extract file for modeling hazard.

```{r filehazardmodeling}
