...
setup_lifetable <- function(ptb){
  ptb %>% mutate(event = str_c("event", event)) %>%
          ...
          mutate(hazard = (event1 / risk_set)) %>% # calculate hazard estimate
          mutate(se_haz = sqrt((hazard * (1 - hazard)) / risk_set)) %>% # se hazard
          group_by(condition) %>%
          mutate(survival = (cumprod(1-hazard)), # calculate survival estimate
                 term     = cumsum(hazard / (risk_set * (1 - hazard))),
                 se_surv  = (survival * sqrt(term))  ) %>% # Greenwood's (1926) approximation
          ungroup() 
}
```
