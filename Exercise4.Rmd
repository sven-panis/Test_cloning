...
print(b2.quartic2) # more iterations
print(b2.quartic3) # more iterations and max_treedepth =12
```
...
TIME−c  specification for period where  c
...
Both transformations extend to the full  
−
∞
  to  
∞
  parameter space. But whereas the logit is symmetric around zero and has a memorable point corresponding to  p=.5  (i.e., 0), the clog-log is asymmetric and has a less-intuitive point corresponding to  
p=.5  (i.e., -0.3665129). Though somewhat odd, the advantage of the clog-log is that
...
```{r}
print(b4_general_interactions_nonlinear)
```

```{r}
b4_general_interactions_nonlinear <- add_criterion(b4_general_interactions_nonlinear, criterion = "waic")

loo_compare(b4_general_nointeractions, b4_general_interactions, b4_general_interactions_nonlinear, criterion = "waic") %>% print(simplify = F)
model_weights(b4_general_nointeractions, b4_general_interactions, b4_general_interactions_nonlinear, weights = "waic") %>% round(digits = 3)
```
...
loo_compare(b4_general_nointeractions, b4_general_interactions, b4_general_interactions_nonlinear,b4_general_interactions_nonlinear_noprop3, criterion = "waic") %>% print(simplify = F)
model_weights(b4_general_nointeractions, b4_general_interactions, b4_general_interactions_nonlinear, b4_general_interactions_nonlinear_noprop3, weights = "waic") %>% round(digits = 3)
```
...
```{r}
residuals(b4_general_interactions_nonlinear) %>% 
  str()
```
...
```{r}
loo(b4_general_interactions_nonlinear)$diagnostics %>% 
  data.frame() %>% 
  glimpse()
```
...
b5_multilevel_3 <-
   brm(data = dataPS,
       ...
       event | trials(1) ~ 0 + Intercept + period_9 + I(period_9^2) + con + con:period_9 + incon +                                 incon:period_9 + trial_c + #I(trial_c^2) + con:trial_c + incon:trial_c + 
                              (1 + period_9|pid),
       prior = c(set_prior("normal(0, 4)", class = "b"),
                 set_prior("cauchy(0,2", class = "sd"),
                 set_prior("lkj(2)", class = "cor")),
         chains = 4, cores = 8, iter = 3000, warmup = 1000,
       control = list(adapt_delta = 0.99), #, stepsize = 0.1, max_treedepth = 20),
         seed = 12,
         file = "models/b5_multilevel_3")
```

```{r}
print(b5_multilevel_3)
```

No transition ended with a divergence, when adapt_delta = .99.
...
b5_multilevel_5 <-
   brm(data = dataPS,
       ...
       control = list(adapt_delta = 0.99), #, stepsize = 0.1, max_treedepth = 20),
         seed = 12, init="0",
         file = "models/b5_multilevel_5")
```

```{r}
print(b5_multilevel_4)
```
