...
visualize("normal(0, 0.5)", "normal(0, 1)", "normal(0, 2)", "normal(0,4)", "skew_normal(-1,3,-0.5)" ,
          xlim = c(-8, 8))
```
...
# logit
tibble(log_odds = rnorm(1e6, mean = 0, sd = 2)) %>% # first prior used 1. 1.5.  2
  ...
  theme(panel.grid = element_blank())
#cloglog
tibble(cloglog = rnorm(1e6, mean = -1, sd = 1.1)) %>%  # good intercept prior
  #sd = 1.7 seems good prior with mean 0, flat except for a density peak at 1
  mutate(p = inv_cloglog(cloglog)) %>% 
  ggplot(aes(x = p)) +
  geom_histogram(bins = 50) +
  scale_y_continuous(NULL, breaks = NULL) +
  theme(panel.grid = element_blank())
...
First make no sense for hazard models...
...
tibble(sd = c(1,1.1,1.2,1.3,1.4,1.5)) %>% 
  mutate(cloglog = map(sd, ~rnorm(1e6, mean = -1, sd = .))) %>% # 1.1 good for intercept intercept
  unnest(cloglog) %>% 
  mutate(sd = str_c("sd = ", sd),
         p  = inv_cloglog(cloglog)) %>% 
...
```{r}
tibble(sigma = c(1,1.1,1.2,1.3,1.4,1.5)) %>% 
  mutate(cloglog = map(sigma, ~rskew_normal(1e6, -1, sigma = ., alpha = -1))) %>% 
  unnest(cloglog) %>% 
  mutate(sigma = str_c("sigma = ", sigma),
         p  = inv_cloglog(cloglog)) %>% 
...
tibble(alpha = c(-8,-7,-6, -5,-4,-3)) %>% 
  mutate(cloglog = map(alpha, ~rskew_normal(1e6, -0.5, sigma = .95, alpha = .))) %>% #!!!!!!! -0.5, .95,-4
  unnest(cloglog) %>% 
...
tibble(alpha = c(-2, -1, 0, 1, 2),
       sigma = c(1,2,3,4,5)) %>% 
  mutate(cloglog = map2(sigma, alpha, ~rskew_normal(1e6, -1, sigma = ., alpha = .))) %>% 
  unnest(cloglog) %>% 
  mutate(sd = str_c("alpha = ", alpha),
         p  = inv_cloglog(cloglog)) %>% 
...
tibble(c = rsn(1e6, xi=-0.59, omega = 1.26, alpha = -4.20) ) %>% # simulate from skew-normal distribution

#tibble(c = rsn(1e6, xi=-1, omega = 1, alpha = -2) ) %>% # simulate from skew-normal distribution
 
   mutate(p = inv_cloglog(c)) %>% 
...
dat <- 
  tibble(c1 = rsn(1e5, -1, 1,-1),
         c2 = rskew_normal(1e5,-1,1,-2)) %>% # use as prior for cloglog intercept !!
