...
cloglog_prob <- cloglog(prob)
logit_prob <- logit(prob)

data <- cbind(prob,cloglog_prob,logit_prob)
data %>% ggplot(aes(x=prob)) +
  geom_line(aes(y=logit_prob), color="green")+
  geom_line(aes(y=cloglog_prob), color="red")
...
#}
# logit
tibble(log_odds = rnorm(1e6, mean = 0, sd = 1.5)) %>% # N(0,1) regularized gently toward .5
...
#cloglog
tibble(clog_log = rnorm(1e6, mean = -1, sd = 1.1)) %>%  
...
tibble(c = rskew_normal(1e6, mu=-0.59, sigma = 1.26, alpha = -4.20) ) %>% # simulate from skew-normal #distribution !!!!!!!! for intercept 
#  !!!!!!!!!
...
Based on your above model assumptions about how ℎ𝑡 relates to time:
...
Prepare effects of conditioning variables on ℎ𝑡
