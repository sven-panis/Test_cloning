...
row1_cond1 <- tibble(condition=factor(1),period=0,`event = 0` = NA,`event = 1` = NA,ev1 = NA,ev0 = NA,RS = Trials_perCondition,
                     hazard = NA,se_haz = NA,survival = 1,term = NA,se_surv = 0,ca = NA,n = NA,se_ca = NA)
row1_cond2 <- tibble(condition=factor(2),period=0,`event = 0` = NA,`event = 1` = NA,ev1 = NA,ev0 = NA,RS = Trials_perCondition,
                     hazard = NA,se_haz = NA,survival = 1,term = NA,se_surv = 0,ca = NA,n = NA,se_ca = NA)
                   
lt_full <- bind_rows(row1_cond1, row1_cond2, lt_full) %>% arrange(condition, period)
...
p3 <-lt_full %>%
  ...
  scale_x_continuous(breaks = 1:(cutoff/bin_size), labels=c(1:(cutoff/bin_size)*bin_size)) +
  scale_y_continuous(limits = c(0,1)) +
  labs(x="Time bin t's endpoint (ms)",
       y="ca(t)",
       colour="Condition") +
  theme(legend.background = element_rect(fill = "transparent"),
        panel.grid = element_blank())
